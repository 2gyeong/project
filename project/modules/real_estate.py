...
def render_real_estate_page():
    ...
    # 탭 생성
    tab_50_60, tab_80_90, tab_average, tab_density = st.tabs(["50~60㎡", "80~90㎡", "동별 평균 매매/전세", "동별 아파트 밀집도"])
...
def calculate_and_display_density(dong_options, area_min, area_max):
    ...
    for dong in dong_options.keys():
        ...
        for apt in apartments:
            ...
            if area_min <= area_value < area_max:
                count += 1
        density_data.append({'법정동': dong, '아파트 수': count})
    ...
    st.write(f"{area_min}~{area_max}㎡ 동별 아파트 밀집도")

    # 파이 차트 시각화
    chart = alt.Chart(df_density).mark_arc().encode(
        theta=alt.Theta(field="아파트 수", type="quantitative"),
        color=alt.Color(field="법정동", type="nominal"),
        tooltip=[alt.Tooltip('법정동:N', title='법정동'), alt.Tooltip('아파트 수:Q', title='아파트 수')]
    ).properties(
        width=700,
        height=400
    )
...
def calculate_and_display_average_prices(dong_options, area_min, area_max):
    ...
    df_combined = pd.concat([df_avg_prices.rename(columns={'평균 매매가': '가격'}),
