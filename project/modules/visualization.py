...
from modules.utils import convert_price, parse_price
...
def create_bar_chart(df, title):
    ...
    chart = alt.Chart(df).mark_bar().encode(
        ...
        y=alt.Y('가격:Q', title="가격 (억)", axis=alt.Axis(format='.0f')),
...
def create_bar_chart(df, title):
    ...
    df['가격_억'] = df['가격'] / 100000000  # 가격을 억 단위로 변환

    # 결측값 제거
    df = df.dropna(subset=['단지명', '가격_억'])

    # Altair 바 차트 생성
    chart = alt.Chart(df).mark_bar().encode(
        x=alt.X('단지명:N', sort='-y', title="단지명"),
        y=alt.Y('가격_억:Q', title="가격 (억)", axis=alt.Axis(format='.0f')),  # 억 단위로 표시
        tooltip=['단지명', alt.Tooltip('가격_억:Q', format=',.2f', title="가격 (억)"), '전용면적', '현재 층/전체 층']
    ).properties(
        title=title,
        width=700,
        height=400
...
# def create_avg_price(df, title):
#     if df.empty:
#         st.write(f"{title} 데이터가 없습니다.")
#         return
    
#     ## 동 별 평균 금액 계산
#     avg_price = df.groupby('동')['가격'].mean().reset_index()
#     avg_price.columns = ['동', '평균 가격']
    
#     chart = alt.Chart(avg_price).mark_bar().encode(
#         x = alt.x('동: N', sort='y', title='동'),
#         y = alt.y('평균 가격: Q', title='평균 가격(억)', axis=alt.Axis(format='.0f')),
#         tooltip = ['동', alt.Tooltip('평균 가격: Q', format=',.2f', title='평균 가격(억)')]   
#     ).properties(
#         title = '동 별 아파트 평균 금액',
#         width=700,
#         height=400
